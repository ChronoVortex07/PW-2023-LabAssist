...
class VideoDisplay(tk.Label):
    def __init__(self, master, scaled: bool = True, consistant_frame_rate: bool = True, keep_aspect: bool = False, progress_bar_callback = None, play_pause_callback = None, *args, **kwargs):
        ...
        self._progress_bar_callback = progress_bar_callback if progress_bar_callback != None else lambda x: None
    ...
    def _load(self, path, pause_after_load=False):
        ...
        with av.open(path) as self._container:
            ...
            self._rotation = self._get_rotation(path)
            ...
            while self._load_thread == current_thread and not self._stop:
                ...
                try:
                    ...
                    # print(self._frame_number % (self._video_info["framerate"] // 2))
                    # call progress bar callback and pass progress in percentage every 0.5 seconds
                    if int(self._frame_number % (self._video_info["framerate"] // 2)) == 0:
                        self._progress_bar_callback(self._time_stamp / self._video_info["duration"])
    ...
    def load(self, path: str):
        ...
        self.stop()
        self.path = path
        self.play(pause_after_load=True)
    ...
    def play(self, pause_after_load=False):
        ...
        if not self._load_thread:
            ...
            self._load_thread = threading.Thread(target=self._load,  args=(self.path, pause_after_load,), daemon=True)
    ...
    def _display_frame(self, event):
        ...
        if self._rotation is not None:
            self._current_img = self._current_img.rotate(self._rotation, expand=True)
    ...
    def _get_rotation(self, file_path_with_file_name):
        metadata = ffmpeg.probe(file_path_with_file_name, cmd="ffmpeg/bin/ffprobe.exe")
        rotation = self._deep_search(metadata, 'rotation')
        if rotation == None:
            rotation = 0
        return rotation
        
        
    def _deep_search(self, data, key):
        if isinstance(data, dict):
            if key in data:
                return data[key]
            for value in data.values():
                result = self._deep_search(value, key)
...
if __name__ == "__main__":
    def callback(progress):
        ...
        pass
    root = tk.Tk()
    root.title("TkinterVideo Demo")
    root.geometry("800x600")

    video = VideoDisplay(root, bg="black", scaled=True, consistant_frame_rate=True, keep_aspect=True, progress_bar_callback=callback)
    video.load(r"dataset\Train\Correct\Zedong_Clear_Correct_4.mp4")
